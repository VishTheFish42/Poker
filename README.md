...
- **AI (`src/poker/ai/`, Python)** — RL agents (PyTorch) that drive the engine through the bindings. The agents, policy network, reward logic, and online training loop are implemented and run against the real engine; `poker.ai.runner` maps seats to agents and plays hands.
...
pytest
```
The AI layer drives the engine through `poker_engine`; `poker.ai.runner.play_hand()` plays a hand with every seat AI-controlled.
