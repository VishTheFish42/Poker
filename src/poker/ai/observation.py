...
Per specs/design.md's "Observation Space": turns a live engine `GameController`,
...
from poker_engine import Card, GameController, Suit, Table, street_number
...
STREET_COUNT = 5  # street_number(): pre-flop, flop, turn, river, showdown
MAX_OPPONENTS = Table.MAX_PLAYERS - 1

_SUITS = list(Suit.__members__.values())
...
def _encode_card(card: Card) -> List[float]:
    suit_one_hot = [1.0 if card.suit == suit else 0.0 for suit in _SUITS]
    return [card.rank_value / 14.0] + suit_one_hot
...
def _encode_scalars(controller: GameController, viewer_seat: int, big_blind: int) -> List[float]:
    ...
    contribution = controller.contributions.get(viewer_seat, 0)
    betting_round = controller.betting_round
    call_amount = betting_round.get_amount_to_call(viewer_seat) if betting_round is not None else 0
...
def _encode_street(table: Table) -> List[float]:
    ...
    street_index = street_number(table.street)
